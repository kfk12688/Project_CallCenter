...
{
	$sql = "INSERT INTO productmaster(productname, productid, producttype, category, series)
	VALUES('{$_POST['productname']}', '{$_POST['productid']}', '{$_POST['producttype']}', '{$_POST['category']}', '{$_POST['series']}')";
...
{
	$sql = mysql_query("DELETE From productmaster WHERE id = '{$_POST['id']}'");
	if($sql)
	{
		echo "success";
	}
...
{
	$sql = "SELECT * FROM productmaster WHERE id = '{$_POST['id']}' ";
	$row = mysql_query($sql);
...
$(function(){
	...
	$('body').delegate('.edit', 'click',function(){
		...
		$.ajax({
			...
			{	
				$('#id').val(show.id);
				$('#pid').val(show.productid);					
				$('#pname').val(show.productname);
				$('#ptype').val(show.producttype);
				$('#category').val(show.category);
				$('#series').val(show.series);		
		...
		<div class="row-fluid">
			...
			<div class="box span5">
					<div class="box-header" data-original-title>
						<h2><i class="halflings-icon edit"></i><span class="break"></span>New Product</h2>
						<div class="box-icon">
							<a href="productmanagement.php" ><i class="halflings-icon refresh"></i></a>
						  ...
						  <fieldset>
							...
							<div class="control-group">
							  ...
							  <label class="control-label" for="typeahead">Product Type</label>	  
								<div class="controls">
								 <input type="text" name="ptype" id="ptype" placeholder="Product Type" >
								</div>
							  </div>
							  <div class="control-group">
							  <label class="control-label" for="typeahead">Brand Name</label>	  
								<div class="controls">
								 <input type="text" name="category" id="category" placeholder="Brand Name" >
								</div>
							  </div>							  
							   <div class="control-group">
							  <label class="control-label" for="typeahead">Model Number</label>	  
								<div class="controls">
								 <input type="text" name="series" id="series" placeholder="Model Number">
								</div>
							  </div>							   							  						
							<div class="form-actions">
							  <a name="save" id="save" class="btn btn-primary">New Product</a>
