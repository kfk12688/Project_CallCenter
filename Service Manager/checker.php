...
{ 
	...
	{	
		...
		$dated = date("Y/m/d");
		$timed = date("h:i:sa");
		$query2 = mysql_query("INSERT INTO temp(assignedby, requestid, engineersid, dated, timed, requestalert)Values('$user', '$assign', '$engineer', '$dated', '$timed', 'YES')")or die(mysql_error());
...
{ 
	...
	{	
		...
		$dated = date("Y/m/d");
		$timed = date("h:i:sa");
		$query2 = mysql_query("UPDATE temp SET engineersid='$engineer', dated='$dated', timed='$timed' where requestid='$assign'")or die(mysql_error());
		mysql_query("UPDATE newrequest SET status='Assigned' where requestid='$assign'")or die(mysql_error());
		echo 0;
	} 
	else
...
{ 
	include("library.php");
	$assignsss=htmlentities($_POST['assign']); 	
	if($query1=mysql_query("UPDATE login SET invoice='$assignsss' where username='$user'")or die(mysql_error()))
	{
		echo 0;
	} 
	else
